...
#define NUM_BUTTONS	10		// On a Pop'n Music controller
...
{
	{ 0, 1, 0x0040, 0 },	// White left
	{ 2, 3, 0x0010, 0 },	// Yellow left
	{ 4, 5, 0x0020, 0 },	// Green left
...
void BuildInfoPacket()
{
	...
	memcpy(InfoPacket.Info.ProductName,
#if POPNMUSIC
			"pop'n music controller        ",
#else
	...
	memcpy(InfoPacket.Info.ProductLicense,
...
void BuildSubPeripheralInfoPacket()
{
	...
	memcpy(SubPeripheralInfoPacket.Info.ProductName,
			"Visual Memory                 ",
			sizeof(SubPeripheralInfoPacket.Info.ProductName));
	memcpy(SubPeripheralInfoPacket.Info.ProductLicense,
			// NOT REALLY! Don't sue me Sega!
			"Produced By or Under License From SEGA ENTERPRISES,LTD.     ",
